    ...
    try {
        ...
        const fullData = data.map(row => {
            ...
            if (numericColumns.includes('需要补货数量')) {
                ...
                newRow['需要补货数量'] = Math.max(0, (100 - estimatedDays - transitDays) * avgDailySales);
            }
            return newRow;
        });
        ...
        headers.forEach(header => {
            ...
            if (numericColumns.includes(header)) {
                ...
                if (header === '平均每天出单') {
                    ...
                    value = Math.max(0, (100 - estimatedDays - transitDays) * avgDailySales);
                    row[header] = value; // 保存计算结果
                }
